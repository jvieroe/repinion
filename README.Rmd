...
Use `epitheme_*()` to quickly apply a tailormode `ggplot` theme to your graphs and data visualizations. This makes them compliant with the company CVI and serves as a time-saver, especially when producing numerous plots. At the moment, two Epinion theme exists: `epitheme_classic()`, designed for most visualization purposes, and `epitheme_map()`, a very minimalist theme designed with geospatial maps in mind.
...
`epitheme_map()` does not take the `gridlines` argument but is otherwise functionally similar.
