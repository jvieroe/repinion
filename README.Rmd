...
* `repinion` contains three different discrete color palettes: `main`, `usered` (the default), and `nosand`. `main` does not included Epinion Red, while `nosand` excludes Warm Sand and is the preferred palette when using `epitheme_dust()`
...
p2 <- ggplot(mtcars, aes(x = wt, y = mpg, color = factor(vs))) +
  geom_point(size = 5, alpha = .95) +
  epitheme_classic(legend = F) +
  color_epi_d(primary = "Red",
              secondary = "Epinion DarkBlue")
